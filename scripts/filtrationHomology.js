...
function initFiltration(){
  ...
  function init() {
    ...
    container2 = document.getElementById('filtrHomology');
    ...
    var lineRed = new THREE.LineBasicMaterial({ color: 0x00ff00, linewidth: 10 });
    var sphereRed = new THREE.MeshStandardMaterial( { color: 0xff0000 } );
    var sphereGreen = new THREE.MeshStandardMaterial( { color: 0x00ff00 } );

    var geoRed = new THREE.SphereGeometry( 0.08, 32, 32 );

    //vertex [0]
    var sp2 = new THREE.Mesh( geoRed, sphereGreen );
    sp2.position.set(-1,-1,0)
    overSpheres[1].push(sp2)
    //vertex [1]
    sp2 = new THREE.Mesh( geoRed, sphereGreen );
    sp2.position.set(0.1,-1,0)
    overSpheres[1].push(sp2)
    //vertex [2]
    sp2 = new THREE.Mesh( geoRed, sphereGreen );
    sp2.position.set(1,-1,0)
    ...
    sp2 = new THREE.Mesh( geoRed, sphereRed );
    sp2.position.set(1,-1,0)
    overSpheres[2].push(sp2)
    sp2 = new THREE.Mesh( geoRed, sphereGreen );
    sp2.position.set(-1,-1,0)
    overSpheres[2].push(sp2)
    //vertex [1]
    ...
    var line = new THREE.Line( lineGeom, lineRed );
    overSpheres[3].push(line);
  ...
  function drawFiltration(){
    ...
    for(var key in overSpheres){
      if(parseInt(key) == limitH){
        for(var i=0; i<overSpheres[key].length; i++){
          scene2.add(overSpheres[key][i])
        }
      }
      else{
        for(var i=0; i<overSpheres[key].length; i++){
          scene2.remove(overSpheres[key][i])
        }
      }
    }
...
function outputUpdate2(filtr) {
	limitH = filtr;

  document.querySelector('.levels').innerHTML = 'Level: '+limitH;
}
